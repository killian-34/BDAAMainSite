<html>
    <head>
    <link rel="stylesheet" type="text/css" href="/css/officers.css">
    <title>BDAA</title>
    </head>
    <body>
        <?php include("nav.html");?>
        <div class="col-sm-8 col-lg-8 fadein">
           <div class="jumbotron main-content padded">
                <div class="container-fluid">
                    <h2 id="officers">Officers</h2>
                    <p class="text-muted">Meet the members of BDAA's Fall 2017 Executive Board</p>
                    <div class="row">
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#shwetaBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/shweta_ambwani.jpg">
                            </a>
                            <p style="color: #BB0000;">Shweta Ambwani<br><span class="text-muted">President</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#jackBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/jack_schroder.jpg">
                            </a>
                            <p style="color: #BB0000;">Jack Schroder<br><span class="text-muted">President Emeritus</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#jtBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/jt_basset.jpg">
                            </a>
                            <p style="color: #BB0000;">J.T. Bassett<br><span class="text-muted">Vice President</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#kalmanBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/kalman_roemer.jpg">
                            </a>
                            <p style="color: #BB0000;">Kalman Roemer<br><span class="text-muted">Educational Director</span></p>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#elizabethBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/elizabeth_gilbert.jpg">
                            </a>
                            <p style="color: #BB0000;">Elizabeth Gilbert<br><span class="text-muted">Event Coordinator</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#obiBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/obi_nnebedum.jpg">
                            </a>
                            <p style="color: #BB0000;">Obi Nnebedum<br><span class="text-muted">Corporate Relations Director</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#marielleBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/marielle_nwana.jpg">
                            </a>
                            <p style="color: #BB0000;">Marielle Nwana<br><span class="text-muted">Internal Relations Director</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#alexBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/alex_lacey.jpg">
                            </a>
                            <p style="color: #BB0000;">Alex Lacey<br><span class="text-muted">Marketing Director</span></p>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#winstonBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/winston_underwood.jpg">
                            </a>
                            <p style="color: #BB0000;">Winston Underwood<br><span class="text-muted">CFO</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#jacksonBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/jack_killian.jpg">
                            </a>
                            <p style="color: #BB0000;">Jackson Killian<br><span class="text-muted">Webmaster</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#johnBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/john_robbins.png.jpg">
                            </a>
                            <p style="color: #BB0000;">John Robbins<br><span class="text-muted">Liaison</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#stacyBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/stacy_comer.jpg">
                            </a>
                            <p style="color: #BB0000;">Stacy Comer<br><span class="text-muted">Liaison</span></p>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#andrewBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/andrew_hay.jpg">
                            </a>
                            <p style="color: #BB0000;">Andrew Hay<br><span class="text-muted">Liaison</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#shiyaoBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/shiyao_zhang.jpg">
                            </a>
                            <p style="color: #BB0000;">Shiyao Zhang<br><span class="text-muted">Liaison</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#oliviaBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/olivia_wan.jpg">
                            </a>
                            <p style="color: #BB0000;">Olivia Wan<br><span class="text-muted">Liaison</span></p>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <a data-toggle="modal" href="#nathanBio">
                                <img class="img-responsive img-rounded" src="/Images/officers/nathan_domeck.jpg">
                            </a>
                            <p style="color: #BB0000;">Nathan Domeck<br><span class="text-muted">Liaison</span></p>
                        </div>
                    </div>
                </div>
            </div>            
        <?php include("footer.html");?>   
        </div>
        <div class="col-sm-2 col-lg-2">
        </div>
        
        
        <!-- Officer Bios -->
        
        <!-- Shweta Bio -->
        <div id="shwetaBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Shweta Ambwani</h4>
                    </div>
                    <div class="modal-body">
                        <p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Data Analytics</p>
                        <p><b>Work Experience:&nbsp;</b>Ford Motor Company, Huntington National Bank, Pure Water Access Project</p>
                        <p><b>Fun Fact:&nbsp;</b>I have a shoe addiction (I own at least 30 pairs)</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Jack Bio -->
        <div id="jackBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Jack Schroder</h4>
                    </div>
                    <div class="modal-body">
                        <!--<p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>GE Aviation, AMEND Consulting, IBM</p>
                        <p><b>Fun Fact:&nbsp;</b>I'm basically Katniss Everdeen with a bow and arrow</p>-->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- JT Bio -->
        <div id="jtBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">J.T. Basset</h4>
                    </div>
                    <div class="modal-body">
                        <p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>Analytics Intern at Cardinal Health in Dublin, OH; Lean Group Engineering Intern at Swagelok Co. in Cleveland, OH</p>
                        <p><b>Fun Fact:&nbsp;</b>My mother is the youngest of twelve girls!</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Kalman Bio -->
        <div id="kalmanBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Kalman Roemer</h4>
                    </div>
                    <div class="modal-body">
                        <!--<p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>GE Aviation, AMEND Consulting, IBM</p>
                        <p><b>Fun Fact:&nbsp;</b>I'm basically Katniss Everdeen with a bow and arrow</p>-->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Elizabeth Bio -->
        <div id="elizabethBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Elizabeth Gilbert</h4>
                    </div>
                    <div class="modal-body">
                        <p><b>Class Rank:&nbsp;</b>2nd Year</p>
                        <p><b>Major:&nbsp;</b>Data Analytics</p>
                        <p><b>Work Experience:&nbsp;</b>Summer Research Fellow at the Mathematical Biosciences Institute</p>
                        <p><b>Fun Fact:&nbsp;</b>My hobbies include cello, photography, and updating my LinkedIn profile.</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Obi Bio -->
        <div id="obiBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Obi Nnebedum</h4>
                    </div>
                    <div class="modal-body">
                        <!--<p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>GE Aviation, AMEND Consulting, IBM</p>
                        <p><b>Fun Fact:&nbsp;</b>I'm basically Katniss Everdeen with a bow and arrow</p>-->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Marielle Bio -->
        <div id="marielleBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Marielle Nwana</h4>
                    </div>
                    <div class="modal-body">
                        <!--<p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>GE Aviation, AMEND Consulting, IBM</p>
                        <p><b>Fun Fact:&nbsp;</b>I'm basically Katniss Everdeen with a bow and arrow</p>-->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Alex Bio -->
        <div id="alexBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Alex Lacey</h4>
                    </div>
                    <div class="modal-body">
                        <!--<p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>GE Aviation, AMEND Consulting, IBM</p>
                        <p><b>Fun Fact:&nbsp;</b>I'm basically Katniss Everdeen with a bow and arrow</p>-->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Winston Bio -->
        <div id="winstonBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Winston Underwood</h4>
                    </div>
                    <div class="modal-body">
                        <!--<p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>GE Aviation, AMEND Consulting, IBM</p>
                        <p><b>Fun Fact:&nbsp;</b>I'm basically Katniss Everdeen with a bow and arrow</p>-->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Jackson Bio -->
        <div id="jacksonBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Jackson Killian</h4>
                    </div>
                    <div class="modal-body">
                        <p><b>Class Rank:&nbsp;</b>5th Year</p>
                        <p><b>Majors:&nbsp;</b>Physics, Computer and Infomration Science</p>
                        <p><b>Work Experience:&nbsp;</b>Undergraduate Research Fellow in Bioinformatics, PNC Software Engineering Intern, Database Designer for small business</p>
                        <p><b>Fun Fact:&nbsp;</b>I have been on the beach of the cliffs of Moher!</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Nathan Bio -->
        <div id="nathanBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Nathan Domeck</h4>
                    </div>
                    <div class="modal-body">
                        <p><b>Class Rank:&nbsp;</b>3rd Year</p>
                        <p><b>Major:&nbsp;</b>Finance</p>
                        <p><b>Minor:&nbsp;</b>Data Analytics</p>
                        <p><b>Work Experience:&nbsp;</b>Cintas Corporation Service Intern, Kohl's Distribution Center Receiving Associate</p>
                        <p><b>Fun Fact:&nbsp;</b>I recently studied abroad in Argentina!</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- John Bio -->
        <div id="johnBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">John Robbins</h4>
                    </div>
                    <div class="modal-body">
                        <!--<p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>GE Aviation, AMEND Consulting, IBM</p>
                        <p><b>Fun Fact:&nbsp;</b>I'm basically Katniss Everdeen with a bow and arrow</p>-->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Stacy Bio -->
        <div id="stacyBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Stacy Comer</h4>
                    </div>
                    <div class="modal-body">
                        <p><b>Class Rank:&nbsp;</b>2nd Year</p>
                        <p><b>Major:&nbsp;</b>Management Information Systems</p>
                        <p><b>Work Experience:&nbsp;</b>Data Analyst intern at Westfield Insurance</p>
                        <p><b>Fun Fact:&nbsp;</b>I've held a shark!</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Andrew Bio -->
        <div id="andrewBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Andrew Hay</h4>
                    </div>
                    <div class="modal-body">
                        <p><b>Class Rank:&nbsp;</b>3rd Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>Command Alkon</p>
                        <p><b>Fun Fact:&nbsp;</b></p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Morgan Bio -->
        <div id="morganBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Morgan Phillips</h4>
                    </div>
                    <div class="modal-body">
                        <!--<p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>GE Aviation, AMEND Consulting, IBM</p>
                        <p><b>Fun Fact:&nbsp;</b>I'm basically Katniss Everdeen with a bow and arrow</p>-->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Shiyao Bio -->
        <div id="shiyaoBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Shiyao Zhang</h4>
                    </div>
                    <div class="modal-body">
                        <!--<p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>GE Aviation, AMEND Consulting, IBM</p>
                        <p><b>Fun Fact:&nbsp;</b>I'm basically Katniss Everdeen with a bow and arrow</p>-->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Tyler Bio -->
        <div id="tylerBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Tyler Poelking</h4>
                    </div>
                    <div class="modal-body">
                        <!--<p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>GE Aviation, AMEND Consulting, IBM</p>
                        <p><b>Fun Fact:&nbsp;</b>I'm basically Katniss Everdeen with a bow and arrow</p>-->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Olivia Bio -->
        <div id="oliviaBio" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Olivia Wan</h4>
                    </div>
                    <div class="modal-body">
                        <!--<p><b>Class Rank:&nbsp;</b>4th Year</p>
                        <p><b>Major:&nbsp;</b>Industrial & Systems Engineering</p>
                        <p><b>Work Experience:&nbsp;</b>GE Aviation, AMEND Consulting, IBM</p>
                        <p><b>Fun Fact:&nbsp;</b>I'm basically Katniss Everdeen with a bow and arrow</p>-->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        
    </body>
<script type="text/javascript">
$( document ).ready(function() {
    ChangeNavActiveClass("#nav-link-officers");
});
    </script>
</html>
